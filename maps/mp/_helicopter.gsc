...
heli_damage_monitor()
{
	...
	for( ;; )
	{
		...
		if( self.damageTaken > self.maxhealth ) {
			...
			// Give player the score to shot down the helicopter and stop the loop
			if ( self.team != attacker.pers["team"] ) {
				playerScore = openwarfare\_scoresystem::getPointsForKill( "MOD_HELIDOWN", type, attacker );
				maps\mp\gametypes\_globallogic::givePlayerScore( playerScore["type"], attacker, self );
				attacker thread maps\mp\gametypes\_rank::giveRankXP( playerScore["type"],  playerScore["score"] );
			}
			break;
		}
	}

...
main(allowed)
{
	...
	switch ( spawnsNeeded ) {
		...
		case "ch":
			if ( !isDefined( getEnt( "ch_trig", "targetname" ) ) ) {
				spawnsNeeded = "sab";
			}
			break;
		...
		case "re":
			...
			break;
		case "vg":
			spawnsNeeded = "sab;tdm;retrieval;sd;dm"; break;
	}
	...
	for ( index = 0; index < mapEntities.size; index++ ) {
		if ( isDefined( mapEntities[index].classname ) && issubstr( mapEntities[index].classname, "_spawn" ) ) {
			...
			if ( deleteSpawnPoint ) {
				spawnsDeleted++;
				mapEntities[index] delete();
			}
		}
	}
	logPrint( "OWFYI;" + spawnsDeleted + " (not used) out of " + spawnsTotal + " spawn entities have been deleted.\n" );	
}
